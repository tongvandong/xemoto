...
public class ModelsController : ControllerBase
{
    ...
    public ModelsController(ICatalogService catalog) => _catalog = catalog;

    [HttpPost]
    public async Task<IActionResult> Create(CreateModelRequest request)
    {
        try { return Ok(new { id = await _catalog.CreateModelAsync(request) }); }
    ...
    {
        try { await _catalog.UpdateModelAsync(id, request); return Ok(new { id }); }
        catch (CatalogException ex) { return BadRequest(new { message = ex.Message }); }
    }
    ...
    {
        try { await _catalog.DeleteModelAsync(id); return Ok(new { message = "Đã xóa dòng xe." }); }
        catch (CatalogException ex) { return BadRequest(new { message = ex.Message }); }
    }
}
